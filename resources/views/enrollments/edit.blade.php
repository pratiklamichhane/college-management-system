            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title">Enrollments Edit</h3>
                    </div>

                    
                    <form class="form-horizontal" action="{{route('enrollments.update' , $enrollment->id)}}" method="post">
                        @method('PUT')
                        @csrf
                        <div class="box-body">
                            <div class="col-md-6">
                                ...
                                <select name="student_id" id="student_id" class="form-control" readonly style="pointer-events: none;">
                                    ...
                                    @foreach($students as $student)
                                        <option value="{{$student->id}}" @if($student->id == $enrollment->student_id) selected @endif>{{$student->user->name}}</option>
                                    @endforeach
                                </select>
                                @error('student_id')
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="course_id" id="course_id" class="form-control">
                                    ...
                                    <option value="{{$course->id}}" @if($course->id == $student->course_id) selected @endif>{{$course->name}}
                                    @endforeach
                                </select>
                                @error('course_id')
